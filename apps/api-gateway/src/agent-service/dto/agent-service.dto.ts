...
export class VerifySignatureDto {
  @ApiProperty({
    description:
      'DID whose authentication key is used to verify the signature. The verification key is resolved from the DID Document server-side.',
    example: 'did:key:z6MkwSMgqptU9M5AiymSSXv3HLhEhXfE6RonJwMSG2dmURdE'
  })
  @IsNotEmpty()
  @IsString()
  did: string;
  ...
  @ApiProperty({
    description: 'The signed data, base64 encoded',
  ...
  @ApiPropertyOptional({
    description: 'Key type hint (informational; the resolved DID key type is authoritative)',
    example: 'ed25519'
  })
  @IsOptional()
  @IsString()
  ...
  @ApiPropertyOptional({
    description: 'Optional DID method hint',
    example: 'key'
  })
  @IsOptional()
  @IsString()
  ...
  @ApiPropertyOptional({
    description: 'Optional public key (ignored for verification — the DID-resolved key is used)',
